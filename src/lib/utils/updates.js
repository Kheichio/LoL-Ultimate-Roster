...
export const UPDATES = [
    ...
    },
    {
        ...
        details: [
            'NEW — your rating can go down. Until now the only thing that ever took a point off you was age, and age does nothing at all before 25, so a nineteen-year-old could play four terrible splits in a row and lose absolutely nothing. A split whose average match rating falls below par now costs you attributes, spread the way age decay spreads them: your hands go first, your game knowledge barely moves. It is bounded for the whole career, so a bad run can set you back but can never grind you into the floor.',
            'NEW — and training is what holds them. Every drill you run on an attribute during a split protects that attribute from that split\'s decline, and three sessions protect it completely. Neglect one and it takes the full hit. This is the point of the whole system: you are not being taxed for a bad split, you are being shown which attributes you stopped looking after. The Training screen marks every attribute Held or Exposed, and tells you exactly what the last split close cost you.',
            'CHANGED — you can now train an attribute that is already at its ceiling. It adds nothing, and that is the point: it is a maintenance session, and it is the only way to protect your best attributes. The old rule refused the drill outright, which meant the attributes you had spent ten years building were the only ones you had no way to defend. At the top of a career your training slots are now a real decision.',
            'NEW — the meta shifts every split. Some champions are strong this split and some are weak, drawn fresh for every split of every year and different for every role, so your pool is never permanently blessed or permanently cursed. It is scored in champion select and in every simmed game, it shows on your draft cards next to the matchup, and there is a panel on the week screen telling you where your own signature currently sits. A strong pick pays exactly what a weak pick costs, so nobody gets a free ride out of it.',
            'FIXED (critical) — contract offers were coming from completely the wrong clubs. Club interest was capped at 100, and a genuinely elite player maxed that cap against almost every club in the world at once — around 95 of the 108. The tie between them was then broken ALPHABETICALLY BY CLUB NAME, and only the first fourteen ever got to make an offer. The practical effect: at 98 rated your offers came from the alphabetically-first fourteen of the weakest teams in the game, T1 scored lower than a Discord amateur side, and the best organisations in the world never entered the draw at all. Interest now falls away once you are clearly better than a club, so the sides chasing you are the ones you would actually want.',
            'NEW — clubs will now refuse you for being too good. An academy will not sign a main-league starter and an amateur side will not sign a 90-rated professional, and both say so in their own words. This only applies once you have played for a real club: nothing about the first rung of a career changed, and an unsigned prospect can still be called by anybody.',
            'FIXED — a free agent had no quality standards at all. The check that stops you signing sideways or downwards only ran while you were under contract, so the moment a deal expired — exactly when an elite player is on the market — every club in the world was fair game again.',
            'NEW — Scrims sharpen the room. Scrimming now permanently raises the four teammates you play alongside, up to +10 each, and you can watch it happen on the Club screen. It makes your club genuinely stronger in matches, not just on paper. The bonus belongs to that club, so it resets if you transfer, and a teammate who gets cut takes their sharpness with them.',
            'NEW — scout any club in the world. Every club name on the league table, on your fixture list and next to your next opponent is now clickable, and opens their actual starting five as cards, with their coach, their strength and a line on who they are. Seeing exactly who holds your role at a club that just called you is the point.',
            'NEW — name a goal club. Pick the team you are trying to reach, and the Transfers screen tells you honestly where you stand with them: how much they rate you, what you still need, whether the window is even open, and whether a rule is flatly refusing you. They also scout you a little harder for it. If you have turned them down twice they will never call again — which was always true and was previously invisible, so you could destroy your own ambition in two clicks and never find out.',
            'NEW — Champion Practice, a weekly activity. Bank games on one champion in your pool without waiting for the schedule to give you the matchup. Pick which champion from your Profile; it defaults to your signature. Every playstyle also has a wider pool to main from than before.',
            'CLEARER — solo queue now tells you the price before you click, not after. Queueing repeatedly inside one week costs health and a losing session puts you on tilt, and until now the only way to discover either was to have already paid. The activity button now shows what the next session will actually cost you, and correctly shows nothing during a burnout bench, where the morale half does not apply.',
            'CLEARER — morale tells you why it moved. A defeat, a bad KDA and a losing run each cost you morale, and all you ever saw was one number with no explanation. Every post-match screen and every simmed result now lists the actual reasons underneath it. The figures are worked out from what you really paid, so it can never claim a penalty that was already absorbed.',
            'FIXED — a losing run followed you between clubs. The run of defeats that costs you extra morale was counted straight off the season block, which still holds your OLD club\'s games for a short window after a transfer — so a bad spell at your previous team was charged against your first game at the new one. It also counted games you watched from the bench while suspended for burnout, which every other penalty in the mode is careful to suppress.',
            'CHANGED — consumables can no longer be spammed. There was exactly one limit in the entire shop: you could buy and drink an unlimited amount of everything, in the same week, with no slot cost. The All-Nighter in particular could grant unlimited extra activities, which made every other limit in the game optional. Items that pump your condition or buy raw attribute points are now capped per week, the Performance Camp is capped for the career, and everything has a sensible bag limit. The shop shows what you have left rather than refusing you at the till. The burnout escape is capped weekly only, and never for the career, so a burnout spiral is always escapable.',
            'NEW — your Profile shows your peak. With attributes that can now fall, "94 overall, peaked at 97" is a real thing worth knowing, so the dossier says it.',
            'BALANCE — measured across full simulated twelve-year careers, not estimated. Decline costs a career about one overall rating point across six under-par splits, and training saves roughly two hundred attribute-shares from it per run — so both halves of the system demonstrably do something. Scrims were deliberately held back from their first tuning after measurement showed a fully-sharpened room quietly inflating everything downstream of it: stronger club, more wins, more awards, more legacy points, more permanent ceiling. Match ratings across the mode are unchanged against the ceiling the game is held to.',
            'NOTE — existing careers are safe. Nothing is reset, re-rolled or reassigned. Careers made before this update simply start with no goal club, no scrim history and nothing lost to decline, and pick all three up from here.',
        ],
    },
    {
        ver: 'Beta 1.7.3',

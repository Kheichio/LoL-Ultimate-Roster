        ...
        return {
            ...
            showcaseCards: (() => {
                ...
                if (picks && picks.length > 0) {
                    return picks.map(uid => get(club).find(c => c.uniqueId === uid)).filter(Boolean);
                }
                return [...get(club)].sort((a, b) => ((b.signature?1000:0)+(b.holographic?500:0)+b.rating) - ((a.signature?1000:0)+(a.holographic?500:0)+a.rating)).slice(0,3);
            })(),
        };
    }
